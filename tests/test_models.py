...
# ---------------------------------------------------------------------------
# HIGH-T-6: LogEntry.from_dict full round-trip
# ---------------------------------------------------------------------------

class TestLogEntryRoundTrip:
    """Full round-trip tests for LogEntry.to_dict / from_dict (HIGH-T-6)."""

    def test_round_trip_full(self):
        """Round-trip a fully-populated LogEntry through to_dict → from_dict."""
        from datetime import timezone

        e = LogEntry(
            raw="2024-01-15 ERROR msg",
            timestamp=datetime(2024, 1, 15, 10, 30, tzinfo=timezone.utc),
            level=LogLevel.ERROR,
            message="msg",
            source=LogSource(file_path="x.log", line_number=42, hostname="host"),
            network=NetworkInfo(source_ip="1.2.3.4", user_agent="Mozilla"),
            http=HTTPInfo(method="GET", path="/api", status_code=200, response_size=512),
            correlation=CorrelationIds(request_id="r1", trace_id="t1", session_id="s1"),
        )
        d = e.to_dict()
        ...
        assert e2.source.file_path == "x.log"
        assert e2.source.line_number == 42
        assert e2.network.source_ip == "1.2.3.4"
        assert e2.http.method == "GET"
        assert e2.http.status_code == 200
        assert e2.correlation.request_id == "r1"

    def test_round_trip_minimal(self):
        """Round-trip a minimal LogEntry (empty) without error."""
        e = LogEntry()
        d = e.to_dict()
        e2 = LogEntry.from_dict(d)
        assert e2.level == LogLevel.UNKNOWN
        assert e2.message == ""

    def test_from_dict_unknown_level_does_not_crash(self):
        """from_dict must handle WARN (alias) and totally unknown level strings."""
        e = LogEntry.from_dict({"level": "WARN"})
        assert e.level == LogLevel.WARNING

        e2 = LogEntry.from_dict({"level": "TOTALLY_FAKE"})
        assert e2.level == LogLevel.UNKNOWN

    def test_from_dict_preserves_http_fields(self):
        """HTTP sub-model survives a full round-trip."""
        http = HTTPInfo(method="POST", path="/submit", status_code=201, response_size=0)
        e = LogEntry(http=http)
        d = e.to_dict()
        e2 = LogEntry.from_dict(d)
        assert e2.http is not None
        assert e2.http.method == "POST"
        assert e2.http.status_code == 201

    def test_from_dict_preserves_network_fields(self):
        """Network sub-model survives a full round-trip."""
        net = NetworkInfo(source_ip="10.0.0.1", user_agent="curl/7.0")
        e = LogEntry(network=net)
        d = e.to_dict()
        e2 = LogEntry.from_dict(d)
        assert e2.network is not None
        assert e2.network.source_ip == "10.0.0.1"
        assert e2.network.user_agent == "curl/7.0"

    def test_from_dict_preserves_correlation(self):
        """Correlation sub-model survives a full round-trip."""
        corr = CorrelationIds(request_id="req-99", session_id="sess-1")
        e = LogEntry(correlation=corr)
        d = e.to_dict()
        e2 = LogEntry.from_dict(d)
        assert e2.correlation.request_id == "req-99"
        assert e2.correlation.session_id == "sess-1"
